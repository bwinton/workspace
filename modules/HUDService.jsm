...
XPCOMUtils.importRelative(this, "NetworkHelper.jsm");
XPCOMUtils.importRelative(this, "PropertyPanel.jsm");
...
function HUD_SERVICE()
{
  ...
  switch (appName()) {
    case "FIREFOX":
      mixins = new FirefoxApplicationHooks();
      break;
    case "THUNDERBIRD":
      mixins = new ThunderbirdApplicationHooks();
      break;
    default:
...
HUD_SERVICE.prototype =
{
  ...
  /**
   * Application-specific current tab getter
   *
   * @returns nsIDOMWindow
   */
  ...
  /**
   * Application-specific way to get a browser for the current tab
   *  
   * @returns a XUL browser
   */
  browserForWindow: function HS_browserForWindow(aWindow) {
    return this.mixins.getBrowserForWindow(aWindow);
  },
  ...
  getChromeWindowFromContentWindow: function () this.currentContext(),
};
 ...
 */
function ThunderbirdApplicationHooks()
{ }

ThunderbirdApplicationHooks.prototype = {
  /**
   * gets the current contentWindow (Thunderbird-specific)
   *
   * @returns nsIDOMWindow
   */
  getCurrentContext: function TAH_getCurrentContext()
  {
    return Services.wm.getMostRecentWindow("mail:3pane");
  },

  getBrowserForWindow: function (aWindow) aWindow.getBrowser(),
};
...
FirefoxApplicationHooks.prototype = {
  ...
  getBrowserForWindow: function (aWindow) aWindow.gBrowser(),
};
...
HeadsUpDisplayUICommands = {
  toggleHUD: function UIC_toggleHUD() {
    ...
    var gBrowser = HUDService.browserForWindow(window);
